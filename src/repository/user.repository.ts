...
export const createUser = async (user: newUser) => {
  ...
  await pool
    ...
    .query(
      "INSERT INTO Users(firstName,lastName,userName,emailAddress,phoneNumber,passwordHash,userRole)Values(@firstName,@lastName,@userName,@emailAddress,@phoneNumber,@passwordHash,@userRole) ",
    );
...
export const setVerificationCode = async (
  ...
  await pool
    ...
    .query(
      "UPDATE Users SET verificationCode=@verificationCode WHERE emailAddress=@emailAddress",
    );
};
export const verifyUserEmail = async (emailAddress: string) => {
  ...
  await pool
    ...
    .input("emailAddress", emailAddress)
    .query(
      "UPDATE Users SET verificationCode=NULL, isVerified=1 WHERE emailAddress=@emailAddress",
    );
...
export const loginUser = async (emailAddress: string, passwordHash: string) => {
  ...
  const userData = await pool
    ...
    .query(
      "SELECT FROM Users WHERE emailAddress = @emailAddress AND passwordHash = @passwordHash",
    );
...
export const updateUserProfile = async (userData: updateUser) => {
  ...
  const result = await pool
    ...
    .query(
      "UPDATE Users SET firstName=@firstName, lastName=@lastName, userName=@userName, emailAddress=@emailAddress, phoneNumber=@phoneNumber, profileImage=@profileImage WHERE emailAddress=@emailAddress",
    );
  return result;
};
